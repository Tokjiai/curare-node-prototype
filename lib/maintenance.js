...
function runDailyMaintenance(db, engine, storeId) {
  ...
  // ⑥ ★2026-09-24追加：LINE webhook受信ログ（webhook_log）の90日より古い行を削除
  //   （お客様のメッセージ本文＝個人情報を含みうるため、保存期間を90日に限る。社長確認済み）
  const webhookCutoff = new Date();
  webhookCutoff.setDate(webhookCutoff.getDate() - WEBHOOK_LOG_RETENTION_DAYS);
  const webhookLogsDeleted = db.prepare(`
    DELETE FROM webhook_log WHERE (store_id = ? OR store_id IS NULL) AND received_at < ?
  `).run(storeId, fmtDate(webhookCutoff) + ' 00:00:00').changes;
...
module.exports = { runDailyMaintenance, WEBHOOK_LOG_RETENTION_DAYS };
