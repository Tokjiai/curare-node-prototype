...
function createCustomerManually(db, storeId, data) {
  ...
  // 新規発行：既存のC0001形式の採番ロジックをfindOrCreateCustomerFromLineと共通化
  const rows = db.prepare(
    `SELECT customer_id FROM customers WHERE store_id = ? AND customer_id LIKE 'C%'`
  ).all(storeId);
  let maxNum = 0;
...
module.exports = { normalizePhoneDigits, findMergeCandidates, mergeCustomerRecords, findOrCreateCustomerFromLine, createCustomerManually };
