...
function registerCustomerFromPublicForm(db, storeId, data) {
  ...
  if (existing) {
    ...
    //
    // ★2026-09-23追加修正：既存行（LINE友だち追加等で顧客IDだけ先に発行済み・
    //   info_confirmed=0）に対する「本登録（氏名等を伴う送信）」は、新規登録
    //   モードと同じ必須項目チェックを適用する。以前はここが「入力された項目
    //   だけ検証」という緩いロジックだったため、氏名・フリガナだけ入力して
    //   電話番号・住所を空欄のまま送信しても登録が完了してしまう不具合があった
    //   （社長の実機テストで発覚：「電話番号や住所が空欄でも予約できてしまう」）。
    //   まだ本登録が済んでいない（info_confirmed偽）お客様が氏名等を伴う送信を
    //   行った場合＝お客様予約フォームの登録カードからの本登録submitとみなし、
    //   新規登録モードと同じ「姓名・フリガナ・電話番号・住所すべて必須」を適用する。
    //   一方、既に本登録済み（info_confirmed真）のお客様が住所等を修正するような
    //   将来的なケースでは、この厳格チェックは不要なため対象外のままにしておく。
    const requiresFullValidation = anyNameFieldGiven && !existing.info_confirmed;
    ...
    } else if (anyNameFieldGiven) {
      // 既に本登録済みの顧客による任意項目の追記・修正：入力された項目だけ検証
      if (lastName || firstName) {
        if (!lastName || !firstName) {
    ...
    // 氏名・電話番号・住所が今回そろった、または既に揃っていれば情報確定とみなす
    const willHaveRealname = fields.realname || existing.realname;
    const willHavePhone = fields.phone || existing.phone;
    const willHaveAddress = fields.address || existing.address;
    if (willHaveRealname && willHavePhone && willHaveAddress) fields.info_confirmed = 1;
