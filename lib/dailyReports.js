...
function getOwnerLineTargets(db, storeId) {
  return db.prepare(`
    ...
    WHERE store_id = ? AND is_owner = 1 AND is_shared_terminal = 0 AND is_active = 1 AND line_user_id IS NOT NULL AND line_user_id != ''
  `).all(storeId);
}
...
async function sendStaffTomorrowSchedule(db, storeId) {
  ...
  const staffLineIds = new Map(
    db.prepare(`SELECT name, line_user_id FROM staff WHERE store_id = ? AND is_active = 1 AND is_shared_terminal = 0 AND line_user_id IS NOT NULL AND line_user_id != ''`)
      .all(storeId)
